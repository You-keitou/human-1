...
type PendingRequest = {
  ...
  sendReasoning: ((text: string) => void) | null
...
export function addPending(
  ...
  sendReasoning: ((text: string) => void) | null = null,
) {
...
export function reasoningPending(requestId: string, text: string): boolean {
  const req = pending.get(requestId)
  if (!req) return false
  if (req.sendReasoning) {
    req.sendReasoning(text)
  } else {
    console.warn('[store] reasoning not supported for this endpoint — dropped')
  }
  return true
}
...
export function resolvePending(requestId: string, finalText: string): boolean {
  ...
  broadcast({ type: 'answered', requestId, content: req.accumulated })
  return true
}
