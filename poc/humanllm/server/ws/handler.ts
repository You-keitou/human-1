...
import { resolvePending, deltaPending, resolveToolPending, reasoningPending } from '../store/pendingRequests'
  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (msg.type === 'response') {
        ...
        const ok = reasoningPending(msg.requestId, msg.content)
        if (ok) {
          broadcast({ type: 'thought', requestId: msg.requestId, content: msg.content })
        } else {
          console.warn(`[ws] no pending request for reasoning id=${msg.requestId}`)
        }
        ...
        if (ok) {
          broadcast({ type: 'tool_called', requestId: msg.requestId, item })
        } else {
